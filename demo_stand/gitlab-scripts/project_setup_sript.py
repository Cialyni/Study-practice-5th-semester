...
class GitLabProjectCreator:
    ...
    def _create_group(
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logging.error(
                    f"Could not create group: {response.status_code} - {response.text}"
                )
            return None
    ...
    def _get_group(self, group_id: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(
                    f"Could not find group with id={group_id}: {response.status_code}"
                )
                return None
        except Exception as e:
    ...
    def _add_user_to_group(self, group_id: int, access_level: int = 40):
        ...
        response = requests.post(
            f"{self.api_url}/groups/{group_id}/members", headers=self.headers, json=data
        )

        if response.status_code == 201:
            logging.info(
                f"Added user {self.user_id} to group {group_id} with access level {access_level}"
            )
        else:
    ...
    def update_dependencies(self, created_modules: List[Dict[str, Any]]):
        ...
        modules_url = {
            m["name"]: m["project"]["http_url_to_repo"] for m in created_modules
        }
