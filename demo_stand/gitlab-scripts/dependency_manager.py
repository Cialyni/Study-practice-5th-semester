...
def get_dependencies_in_group(api_url: str, headers: Dict, timeout: int, group_id: int) -> Dict[str, List[str]]:
    try:
        ...
        if response.status_code == 200:
            logging.info(
                f"get all project in group with id={group_id}"
            )
            for project in projects_response:
                project_toml = get_pyproject_toml(api_url, headers, timeout, project["id"])
                ...
                dependencies[project['name']] = [dep.split('@')[0][:-1] for dep in project_dependencies]
        else:
            logging.error(
