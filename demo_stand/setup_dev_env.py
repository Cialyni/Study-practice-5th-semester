...
def send_token_to_webhook(token):
    try:
        response = requests.post(
            "http://localhost:8000/reload-token", 
            json={"new_token": token}, 
            timeout=2
        )
...
class TestStandSetup:
    ...
    def setup_nexus(self):
        ...
        self.wait_for_service(f"{self.nexus_url}/service/rest/v1/status", "Nexus API", 60)
        ...
        response = session.post(
            f"{self.nexus_url}/service/rest/v1/repositories/pypi/hosted",
            json=repo_data
        )
        ...
        webhook_response = session.post(
            f"{self.nexus_url}/service/rest/v1/capabilities",
            json=webhook_data
        )
    ...
    def setup_group_variables(self, group_id):
        ...
        variables = [
            {
                'key': 'NEXUS_URL', 
                'value': 'http://nexus:8081/repository/test-repo/'
            },
            {
                'key': 'NEXUS_USER', 
                'value': 'admin'
    ...
    def create_env_file(self, root_pass, project_id, group_id):
        ...
        with open('.env', 'w') as f:
            f.write(f"""GITLAB_ACCESS_TOKEN={self.gitlab_token}
